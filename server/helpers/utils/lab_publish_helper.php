...
function hackme_insert_lab_catalog_card_from_proposal_fields(
    ...
    if ($solution !== '') {
        $solEsc = $conn->real_escape_string($solution);
        $conn->query("UPDATE labs SET solution='$solEsc' WHERE lab_id=$newLabId");
    }

    $hintsLines = array_filter(array_map('trim', explode("\n", $hintsRaw)));
    if (!empty($hintsLines)) {
        $titleEscForChall = $conn->real_escape_string($title !== '' ? $title : 'Challenge 1');
        $conn->query("INSERT INTO challenges (lab_id, created_by, title, statement, is_active) VALUES ($newLabId, $createdByUserId, '$titleEscForChall', '', 1)");
        $challengeId = (int)$conn->insert_id;
        if ($challengeId > 0) {
            foreach ($hintsLines as $hText) {
                if ($hText !== '') {
                    $hTextEsc = $conn->real_escape_string($hText);
