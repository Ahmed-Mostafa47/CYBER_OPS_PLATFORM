        ...
        if ($basePath !== '' && $folder !== '') {
            ...
            if ($labPath && is_dir($labPath)) {
                if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                    $cmd = 'start /B cmd /C "cd /d "' . $labPath . '" && (docker compose down >nul 2>&1 || docker-compose down >nul 2>&1)"';
                    pclose(popen($cmd, 'r'));
                } else {
                    $cmd = 'cd "' . addslashes($labPath) . '" && (docker compose down >/dev/null 2>&1 || docker-compose down >/dev/null 2>&1) &';
                    exec($cmd);
                }
            }
        }
